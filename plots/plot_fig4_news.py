...
Usage: python plot_fig4_news.py
Time: ~1M
"""
...
if __name__ == '__main__':
    ...
    print('>>> Start to extract engagement map for news and plot...')
    ...
    if to_plot:
        ...
        ax1.legend([plt.Rectangle((0, 0), 1, 1, fc=cornflower_blue), plt.Rectangle((0, 0), 1, 1, fc=tomato)],
                   ['Tweeted News', 'Top News'], loc='upper right', fontsize=24, frameon=False)
        ...
        max_xlim = 1.2 * max([tweeted_kde_y(y).max(), quality_kde_y(y).max()])
        ...
        max_ylim = 1.2 * max([tweeted_kde_x(x).max(), quality_kde_x(x).max()])
        ...
        plt.savefig('../images/fig4_news_emap.pdf', bbox_inches='tight')
        plt.show()
