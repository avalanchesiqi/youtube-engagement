...
Usage: python extract_channel_reputation.py -i ../ -o ./output/train_channel_relative_engagement.txt
Time: ~3M
"""
    ...
    for subdir, _, files in os.walk(train_loc):
        for f in files:
            with open(os.path.join(subdir, f), 'r') as fin:
                fin.readline()
                for line in fin:
                    _, _, _, _, _, _, channel_id, _, _, _, _, re30, _ = line.rstrip().split('\t', 12)
                    output_data.write('{0}\t{1}\n'.format(channel_id, re30))
