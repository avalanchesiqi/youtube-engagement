...
""" Predict relative engagement from channel reputation.
...
def _load_data(filepath):
    ...
    with open(filepath, 'r') as fin:
        ...
        for line in fin:
            row = np.zeros(10)
            vid, _, duration, _, _, _, channel_id, _, _, _, _, re30, _ = line.rstrip().split('\t', 12)
            if channel_id in channel_re_dict:
                row[0] = np.log10(int(duration))
                # training dataset spans over 51 days
                row[1] = len(channel_re_dict[channel_id]) / 51
                row[2] = np.mean(channel_re_dict[channel_id])
                row[3] = np.std(channel_re_dict[channel_id])
                row[4] = np.min(channel_re_dict[channel_id])
                row[5] = np.percentile(channel_re_dict[channel_id], 25)
                row[6] = np.median(channel_re_dict[channel_id])
                row[7] = np.percentile(channel_re_dict[channel_id], 75)
                row[8] = np.max(channel_re_dict[channel_id])
                row[9] = float(re30)
                matrix.append(row)
                vids.append(vid)
    return matrix, vids
