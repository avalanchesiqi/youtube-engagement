            ...
            for channel_id in test_channel_list:
                if channel_id in train_channel_list and len(train_channel_cluster[channel_id]) >= k:
                    ...
                    for train_video in train_videos:
                        ...
                        vid, publish, duration, definition, category, detect_lang, _, topics, _, _, _, re30, _ = train_video.rstrip().split('\t', 12)
                    ...
                    for test_video in test_videos:
                        row = np.zeros(1+1+category_cnt+lang_cnt+topic_cnt+1)
                        vid, publish, duration, definition, category, detect_lang, _, topics, _, _, _, re30, _ = test_video.rstrip().split('\t', 12)
                        row[0] = np.log10(int(duration))
                        if definition == '1':
                            row[1] = 1
                        if category in category_dict:
