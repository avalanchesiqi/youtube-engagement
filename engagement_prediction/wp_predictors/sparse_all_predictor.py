...
""" Predict watch percentage from all features in sparse matrix.
...
def _load_data(filepath):
    ...
    with open(filepath, 'r') as fin:
        fin.readline()
        for line in fin:
            vid, _, duration, definition, category, detect_lang, channel_id, topics, _, _, wp30, _, _ = line.rstrip().split('\t', 12)
            if channel_id in channel_wp_dict:
                reputation_vector = strify([len(channel_wp_dict[channel_id])/51,
                                            np.mean(channel_wp_dict[channel_id]),
                                            np.std(channel_wp_dict[channel_id]),
                                            np.min(channel_wp_dict[channel_id]),
                                            np.percentile(channel_wp_dict[channel_id], 25),
                                            np.median(channel_wp_dict[channel_id]),
                                            np.percentile(channel_wp_dict[channel_id], 75),
                                            np.max(channel_wp_dict[channel_id])])
                row = [vid, duration, definition, category, detect_lang, topics, reputation_vector, wp30]
                matrix.append(row)
    return matrix
...
def vectorize_train_data(data):
    ...
    for _, duration, definition, category, detect_lang, topics, reputation_vector, wp30 in data:
        if not (topics == '' or topics == 'NA'):
...
def vectorize_test_data(data, topic_dict):
    ...
    for vid, duration, definition, category, detect_lang, topics, reputation_vector, wp30 in data:
        if not (topics == '' or topics == 'NA'):
