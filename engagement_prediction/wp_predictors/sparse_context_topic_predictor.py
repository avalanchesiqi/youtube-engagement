...
""" Predict watch percentage from video context and freebase topics in sparse matrix.
...
def vectorize_train_data(data):
    ...
    for _, duration, definition, category, detect_lang, topics, wp30 in data:
        if not (topics == '' or topics == 'NA'):
...
def vectorize_test_data(data, topic_dict):
    ...
    for vid, duration, definition, category, detect_lang, topics, wp30 in data:
        if not (topics == '' or topics == 'NA'):
...
if __name__ == '__main__':
    ...
    print('>>> Start to predict watch percentage with video context and freebase topics in sparse matrix...')
